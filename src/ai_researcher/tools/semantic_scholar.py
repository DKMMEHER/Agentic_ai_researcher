...
def semantic_scholar_search(query: str, max_results: int = 5) -> str:
    ...
    try:
        fields = (
            "title,authors,year,citationCount,influentialCitationCount,abstract,url"
        )
        ...
        logger.info(
            "Retrieved %d highly-cited results from Semantic Scholar.", len(results)
        )
        ...
        raise SemanticScholarError(
            message=f"Processing Error: {e!s}", query=query
        ) from e
...
def semantic_scholar_citations(paper_id: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        logger.exception(
            "Semantic Scholar API failed fetching citations for: '%s'", paper_id
        )
...
def semantic_scholar_references(paper_id: str, max_results: int = 5) -> str:
    ...
    try:
        ...
        logger.exception(
            "Semantic Scholar API failed fetching references for: '%s'", paper_id
        )
        if getattr(e.response, "status_code", None) == 404:
            return f"API Error: Paper ID '{paper_id}' not found in Semantic Scholar."
        return f"API Error: Request failed ({e!s})."
