...
def wikipedia_search(query: str) -> str:
    ...
    try:
        ...
        try:
            ...
            summary = wikipedia.summary(top_page_title, sentences=10, auto_suggest=False)
            
            result = f"--- WIKIPEDIA SUMMARY: {top_page_title} ---\n"
            result += summary + "\n\n"
            result += f"Other related pages you could search: {', '.join(search_results[1:])}"
            return result
            
        except wikipedia.exceptions.DisambiguationError as e:
            # Handle ambiguous search terms (e.g. "Mercury" -> Planet, Element, God)
            logger.warning("Wikipedia DisambiguationError for: '%s'. Options: %s", top_page_title, e.options[:5])
