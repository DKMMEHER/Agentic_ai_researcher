...
def youtube_transcript_reader(url: str) -> str:
    ...
    try:
        ...
        elif parsed_url.hostname in ("youtube.com", "www.youtube.com"):  # noqa: SIM102
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if available:
                    ...
                    logger.info(
                        "Using %s transcript (language: %s)",
                        "auto-generated"
                        if getattr(best, "is_generated", False)
                        else "manual",
                        transcript_lang,
                    )
        ...
        if len(full_text) > 30000:
            logger.warning(
                "Truncating YouTube transcript from %d to 30000 chars", len(full_text)
            )
            full_text = (
        ...
        logger.info(
            "Successfully extracted %d characters from transcript (lang=%s)",
            len(full_text),
            transcript_lang,
        )
        return full_text
        ...
        raise YoutubeTranscriptError(
            message=f"Failed to extract transcript: {e!s}", video_id=video_id
        ) from e
