...
def read_pdf(url: str) -> str:
    ...
    try:
        ...
        docs = [Document(page_content=chunk, metadata={"source": url, "doc_id": doc_id}) for chunk in chunks]
        vector_store = get_vector_store()
        vector_store.add_documents(docs)
        
        # --- Compile the LLM Report ---
        logger.info("Successfully ingested %d text chunks and %d valid figures from PDF.", len(chunks), len(extracted_figures))
