...
async def _call_supervisor(state: AgentState) -> dict:
    ...
    try:
        ...
        if prediction:
            ...
            logger.warning("Supervisor parsed prediction is None. Defaulting to 'research_paper'.")
            intent = "research_paper"
            
        if getattr(raw_msg, "usage_metadata", None):
            usage = raw_msg.usage_metadata
    except Exception as e:
        logger.error("Supervisor classification failed: %s. Defaulting to 'research_paper'.", e)
        intent = "research_paper"
    ...
    if intent == "direct_chat":
        content = prediction.chat_response or "How can I help you with your research today?"
