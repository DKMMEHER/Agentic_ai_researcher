...
def agent_target(inputs: dict) -> dict:
    ...
    try:
        ...
        scores = {"accuracy": 0, "completeness": 0, "coherence": 0, "citation_quality": 0, "depth": 0}
...
def citation_quality_evaluator(run, example) -> dict:
    return {"key": "citation_quality", "score": run.outputs.get("citation_quality", 0) / 5.0}
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run LangSmith LLM-as-a-Judge evaluation")
    parser.add_argument("--experiment-name", type=str, default=None)
    parser.add_argument("--judge-provider", type=str, default="gemini", choices=["gemini", "groq"])
